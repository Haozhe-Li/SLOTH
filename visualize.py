...
def plot_model_metrics(model_results):
    ...
    avg_times = [
        data["average_metrics"]["avg_response_time"] for data in model_results.values()
    ]
    pass_rates = [
        data["evaluation_results"]["pass@1"] for data in model_results.values()
    ]
    ttft = [
        data["average_metrics"]["avg_time_to_first_token"]
...
def build_regression_models(model_results):
    ...
    for model_name, data in model_results.items():
        ...
        output_tokens = np.array(
            [metric["output_tokens"] for metric in problem_metrics]
        )
        total_tokens = input_tokens + output_tokens
        ttft = np.array([metric["time_to_first_token"] for metric in problem_metrics])
        response_time = np.array(
            [metric["response_time"] for metric in problem_metrics]
        )
    ...
    if len(all_models_data) > 1:
        ...
        for i, (model_name, data) in enumerate(all_models_data.items()):
            ...
            plt.scatter(
                input_tokens, ttft, alpha=0.3, color=color, label=f"{model_name} data"
            )
            ...
            plt.plot(
                x_sorted,
                reg.predict(x_sorted.reshape(-1, 1)),
                color=color,
                linewidth=2,
                label=f"{model_name} (R² = {score:.3f})",
            )
        ...
        combined_response_path = os.path.join(
            static_dir, "combined_response_time_regression.png"
        )
...
def cleanup_static_files():
    """Clean up all static images when the application is closed"""
    static_dir = os.path.join(os.getcwd(), "static")
    if os.path.exists(static_dir):
        try:
            shutil.rmtree(static_dir)
            print(f"Successfully cleaned up static files directory: {static_dir}")
        except Exception as e:
            print(f"Error cleaning up static files: {e}")
...
def launch_gradio_interface():
    ...
    # Define interface
    with gr.Blocks(
        title="LLM Evaluation Visualizer", css="#gallery { min-height: 400px; }"
    ) as demo:
        ...
        output_markdown = gr.Markdown(
            "Select models and click 'Generate Visualizations' to start"
        )
    ...
    # Launch interface and register cleanup to happen on app close
    demo.launch(share=False)

    # Clean up static files after the app is closed
